...
Do not use markdown headers larger than h3. Keep your advice structured but conversational. Give a recipe if asked, or customize a recipe if the user states their health goals.`;
  ...
  // API Endpoint: Send OTP (Production Email dispatch only)
  app.post("/api/send-otp", async (req, res) => {
    try {
      ...
      if (type !== "email") {
        return res.status(400).json({ error: "Only production email delivery channel is supported." });
      }
      ...
      if (!smtpUser || !smtpPass) {
        return res.status(400).json({ 
          error: "ईमेल सर्वर (SMTP) कॉन्फ़िगर नहीं है! / SMTP credentials are not configured in the environment settings. Please specify your SMTP_USER and SMTP_PASS in settings." 
        });
      }
      ...
      return res.json({
        success: true,
        message: "OTP successfully dispatched to your email inbox."
      });
    } catch (e: any) {
      console.error("OTP production email dispatch error:", e);
      return res.status(500).json({ 
        error: `ईमेल भेजने में असमर्थ / Failed to dispatch email. Details: ${e.message || "SMTP error"}` 
      });
    }
  });
